...
//   stopSimulation: () => void;
//   addManualReport: (location: string, description: string, resolvedCoords?: { lat: number; lng: number }) => void;
//   addImageReport: (location: string, description: string, resolvedCoords?: { lat: number; lng: number }) => void;
...
//   const processData = useCallback((sourceId: string, sourceType: 'tweet' | 'image' | 'manual_report', content: string, location: string, resolvedCoords?: { lat: number; lng: number }) => {
...
//     const alert = createAlert(sourceId, sourceType, location, content, finalClassification, resolvedCoords);
...
//   const addManualReport = useCallback((location: string, description: string, resolvedCoords?: { lat: number; lng: number }) => {
//     const id = `report-${Date.now()}`;
//     const feedItem: FeedItem = { id, type: 'manual_report', content: description, timestamp: new Date(), location };
//     setFeedItems(prev => [feedItem, ...prev].slice(0, 50));
//     processData(id, 'manual_report', description, location, resolvedCoords);
//   }, [processData]);

//   const addImageReport = useCallback((location: string, description: string, resolvedCoords?: { lat: number; lng: number }) => {
//     const id = `image-${Date.now()}`;
//     const feedItem: FeedItem = { id, type: 'image', content: description, timestamp: new Date(), location };
//     setFeedItems(prev => [feedItem, ...prev].slice(0, 50));
//     processData(id, 'image', description, location, resolvedCoords);
//   }, [processData]);
...
export function SimulationProvider({ children }: { children: React.ReactNode }) {
  ...
  const clearAll = useCallback(async () => {
    // Wipe all state immediately so UI clears at once
    setAlerts([]);
    setFeedItems([]);
    setProcessingLogs([]);
    setDispatchedReports([]);
    setTeamResponses([]);
    setDispatchLogs([]);
    // Persist the clear to the backend so refresh stays clean
    try {
      await fetch('http://localhost:4000/api/reset', { method: 'POST' });
    } catch (err) {
      console.warn('Backend reset failed — UI cleared but data may reload on refresh:', err);
    }
  }, []);
