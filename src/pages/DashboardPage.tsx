...
import {
  ...
  MapPin, Clock, Send, CheckCircle, MessageSquare, ChevronDown, ChevronUp, Trash2
} from 'lucide-react';
...
export default function DashboardPage() {
  const { filteredAlerts, selectedRole, setSelectedRole, isSimulationRunning, alerts, dispatchedReports, addDispatchReport, clearAll } = useSimulation();
    ...
    <div className="min-h-screen p-6">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          {/* Right side: dispatched badge + clear button */}
          <div className="flex items-center gap-3">
            ...
            {(filteredAlerts.length > 0 || dispatchedReports.length > 0) && (
              confirmClear ? (
                <div className="flex items-center gap-2 bg-destructive/10 border border-destructive/30 rounded-lg px-3 py-1.5">
                  <span className="text-xs text-destructive font-medium">Clear everything?</span>
                  <button
                ...
                <Button
                  variant="outline"
                  size="sm"
                  className="gap-1.5 text-xs border-destructive/30 text-destructive/70 hover:border-destructive hover:text-destructive hover:bg-destructive/5 transition-colors"
